...
def load(data_path, session):
    ...
    for key in data_dict:
        with tf.variable_scope(key, reuse=True):
            session.run( tf.get_variable( 'weights' ), data_dict[ key ][ 'weights' ] )
            session.run( tf.get_variable( 'biases' ), data_dict[ key ][ 'biases' ] )      
...
def load_with_skip(data_path, session, skip_layer):
    ...
    for key in data_dict:
        if key not in skip_layer:
            with tf.variable_scope( 'cleannetwork' ) :
                with tf.variable_scope(key, reuse=True):
            ...
            with tf.variable_scope( 'dirtynetwork' ) :
                with tf.variable_scope(key, reuse=True):
                    session.run( tf.get_variable( 'weights' ).assign( data_dict[ key ][ 'weights' ] ) )
                    session.run( tf.get_variable( 'biases' ).assign( data_dict[ key ][ 'biases' ] ) )      
               # for subkey, data in zip(('weights', 'biases'), data_dict[key]):
...
def load_with_skip_siamense( data_path, session, skip_layer ):
    data_dict = np.load(data_path).item()
    for key in data_dict:
        if key not in skip_layer:
            with tf.variable_scope(key, reuse=True):
